...
public class DailyReportService : IDailyReportService
{
    ...
    public async Task SendDailyReportAsync()
    {
        ...
        var stats = new DailyReportStats(
            ...
            TotalSponsorRevenue: paidSponsors.Sum(s => (s.CustomAttestationTotal.HasValue ? (decimal)s.CustomAttestationTotal.Value : SponsorPackagePrice(s.Package) + s.ExtraEtenPartyCount * 50m + s.ExtraDrankkaart20Count * 20m)));
    ...
    private static void BuildSponsorsSheet(XLWorkbook workbook, IReadOnlyList<SponsorRequestEntity> sponsors)
    {
        ...
        {
            var total = sponsor.CustomAttestationTotal.HasValue
                ? sponsor.CustomAttestationTotal.Value
                : (double)(SponsorPackagePrice(sponsor.Package) + sponsor.ExtraEtenPartyCount * 50m + sponsor.ExtraDrankkaart20Count * 20m);
